...
typedef struct H5VL_dtio_params_t
{
  bool dtio_initialized;
} H5VL_dtio_params_t;
...
static herr_t H5VL_dtio_file_close_helper (H5VL_dtio_file_t *file,
                                           hid_t dxpl_id, void **req);
...
H5VL_dtio_group_traverse (H5VL_dtio_item_t *item, char *path, hid_t dxpl_id,
                          void **req, char **obj_name, void **gcpl_buf_out,
...
static herr_t H5VL_dtio_build_io_op_match (hid_t file_space_id,
                                           size_t type_size, size_t tot_nelem,
                                           void *rbuf, const void *wbuf,
                                           task read_op, task write_op);
...
H5VL_dtio_info_copy (const void *_old_info)
{
  ...
  if (NULL
      == (new_info = (H5VL_dtio_info_t *)malloc (sizeof (H5VL_dtio_info_t))))
...
H5VL_dtio_dataset_create (void *_item,
                          const H5VL_loc_params_t H5VL_ATTR_UNUSED *loc_params,
                          const char *name, hid_t H5VL_ATTR_UNUSED lcpl_id,
                          ...
                          hid_t dapl_id, hid_t dxpl_id, void **req)
{
  ...
  if (H5VL_dtio_oid_create (item->file, item->file->max_oid + (uint64_t)1,
                            H5I_DATASET, &dset->obj.bin_oid, &dset->obj.oid)
      < 0)
  ...
  return (void *)dset
         ...
         /* Close target group */
         if (target_grp
             && H5VL_dtio_group_close (target_grp, dxpl_id, req) < 0) std::cerr
         < < < "can't close group";
...
/*---------------------------------------------------------------------------*/
static void *
H5VL_dtio_dataset_open (void *_item, const H5VL_loc_params_t *loc_params,
  ...
  if (!collective || (item->file->my_rank == 0))
    {
      ...
      if (md_len + sizeof (uint64_t) > sizeof (dinfo_buf_static))
        {
          if (NULL
              == (dinfo_buf_dyn
                  = (uint8_t *)malloc (md_len + sizeof (uint64_t))))
      ...
      if ((ret = H5VL_dtio_read (dset->obj.oid,
                                 (char *)(dinfo_buf + sizeof (uint64_t)),
                                 md_len, 0))
          < 0)
      ...
      if (collective && (item->file->num_procs > 1))
        {
          ...
          if (MPI_SUCCESS
              != MPI_Bcast ((char *)dinfo_buf, sizeof (dinfo_buf_static),
          ...
          if (dinfo_buf != dinfo_buf_static)
            {
              assert (md_len + sizeof (uint64_t) > sizeof (dinfo_buf_static));
              if (MPI_SUCCESS
                  != MPI_Bcast ((char *)dinfo_buf + (4 * sizeof (uint64_t)),
  ...
  else
    {
      ...
      if (MPI_SUCCESS
          != MPI_Bcast ((char *)dinfo_buf, sizeof (dinfo_buf_static), MPI_BYTE,
                        0, item->file->comm))
        std::cerr << "can't bcast dataset info";
      ...
      if (tot_len + (4 * sizeof (uint64_t)) > sizeof (dinfo_buf_static))
        {
          ...
          /* Receive dataset info */
          if (MPI_SUCCESS
              != MPI_Bcast ((char *)dinfo_buf, (int)tot_len, MPI_BYTE, 0,
                            item->file->comm))
            std::cerr << "can't bcast dataset info (second bcast)";
  ...
  for (dsetnum = 0; dsetnum < count; dsetnum++)
    {
      ...
      if (H5VL_dtio_tconv_init (dset->type_id, &file_type_size, mem_type_id,
                                &mem_type_size, &types_equal, &reuse,
                                &need_bkg, &fill_bkg)
          < 0)
      ...
      switch (H5Pget_layout (dset->dcpl_id))
        ...
        case H5D_CONTIGUOUS:
          if (NULL
              == (chunk_info = (H5VL_dtio_select_chunk_info_t *)malloc (
          ...
          memset (chunk_info->chunk_coords, 0,
                  sizeof (chunk_info->chunk_coords));
        ...
        case H5D_CHUNKED:
          ...
          if (H5VL_dtio_get_selected_chunk_info (
                  dset->dcpl_id, real_file_space_id, real_mem_space_id,
                  &chunk_info, &chunk_info_len)
              < 0)
      ...
      if ((num_elem_chunk
           = H5Sget_simple_extent_npoints (chunk_info[0].fspace_id))
          < 0)
      ...
      for (i = 0; i < chunk_info_len; i++)
        {
          ...
          /* Create chunk key */
          if (H5VL_dtio_oid_create_chunk (
                  dset->obj.item.file, dset->obj.bin_oid, ndims,
                  chunk_info[i].chunk_coords, &chunk_oid)
              < 0)
            ...
            if (types_equal)
              {
                ...
                if (mem_space_id == H5S_ALL && chunk_info_len == 1)
                  if ((match_select = H5Sextent_equal (
                           real_file_space_id, chunk_info[i].fspace_id))
                      < 0)
                    HGOTO_ERROR (
                        H5E_DATASPACE, H5E_CANTCOMPARE, FAIL,
                ...
                if (match_select)
                  {
                    ...
                    if (H5VL_dtio_build_io_op_match (
                            chunk_info[i].fspace_id, file_type_size,
                            (size_t)num_elem, buf, NULL, read_op, NULL)
                        < 0)
                ...
                else
                  {
                    ...
                    if (H5VL_dtio_build_io_op_merge (
                            ...
                            file_type_size, (size_t)num_elem, buf, NULL,
                            read_op, NULL)
                        < 0)
                      std::cerr << "can't generate DTIO read op";
                  } /* end else */
                ...
                if ((ret = dtio_read_op_operate (read_op, chunk_oid,
                                                 LIBDTIO_OPERATION_NOFLAG))
                    < 0)
                  std::cerr << "can't read data from dataset: "
                            << strerror (-ret);
              } /* end if */
            else
              {
                ...
                if (H5Ssel_iter_get_seq_list (sel_iter_id, (size_t)1,
                                              (size_t)-1, &nseq_tmp,
                                              &nelem_tmp, &sel_off, &sel_len)
                    < 0)
                ...
                if (need_bkg)
                  {
                    if (contig && (reuse == H5VL_DTIO_TCONV_REUSE_BKG))
                    ...
                    else
                      {
                        if (!tmp_bkg_buf)
                          if (NULL
                              == (tmp_bkg_buf = malloc ((size_t)num_elem_chunk
                                                        * mem_type_size)))
                ...
                if (H5VL_dtio_build_io_op_contig (
                        chunk_info[i].fspace_id, file_type_size,
                        (size_t)num_elem, tconv_buf, NULL, read_op, NULL)
                    < 0)
                ...
                if ((ret = dtio_read_op_operate (read_op, chunk_oid,
                                                 LIBDTIO_OPERATION_NOFLAG))
                    < 0)
                  std::cerr << "can't read data from dataset: %s",
                      strerror (-ret);
                ...
                if (fill_bkg && (bkg_buf == tmp_bkg_buf))
                  if (H5Dgather (chunk_info[i].mspace_id, buf, mem_type_id,
                                 (size_t)num_elem * mem_type_size, bkg_buf,
                                 NULL, NULL)
                      < 0)
                ...
                if (tconv_buf == tmp_tconv_buf)
                  {
                    ...
                    if (H5Dscatter (H5VL_dtio_scatter_cb, &scatter_cb_ud,
                                    mem_type_id, chunk_info[i].mspace_id, buf)
                        < 0)
...
static herr_t
H5VL_dtio_dataset_write (size_t count, void *_dset[], hid_t mem_type_id[],
                         hid_t mem_space_id[], hid_t file_space_id[],
  ...
  for (dsetnum = 0; dsetnum < count; dsetnum++)
    {
      ...
      if (H5VL_dtio_tconv_init (dset->type_id, &file_type_size,
                                mem_type_id[dsetnum], &mem_type_size,
                                &types_equal, &reuse, &need_bkg, &fill_bkg)
          < 0)
        std::cerr << "can't initialize type conversion";
      ...
      if (need_bkg)
        if (NULL
            == (bkg_buf = malloc ((size_t)num_elem_chunk * mem_type_size)))
          std::cerr << "can't allocate background buffer";
            ...
            if (types_equal)
                ...
                if (match_select)
                  {
                    ...
                    if (H5VL_dtio_build_io_op_match (
                            chunk_info[i].fspace_id, file_type_size,
                            (size_t)num_elem, NULL, buf[dsetnum], NULL,
                            write_op)
                        < 0)
                      std::cerr << "can't generate DTIO write op";
                  } /* end if */
            ...
            else
              {
                ...
                if (fill_bkg)
                  {
                    ...
                    if (H5VL_dtio_build_io_op_contig (
                            chunk_info[i].fspace_id, file_type_size,
                            (size_t)num_elem, bkg_buf, tconv_buf, read_op,
                            write_op)
                        < 0)
                      std::cerr << "can't generate DTIO write op";
                  ...
                  if (H5VL_dtio_build_io_op_contig (
                          chunk_info[i].fspace_id, file_type_size,
                          (size_t)num_elem, NULL, tconv_buf, NULL, write_op)
                      < 0)
                    std::cerr << "can't generate DTIO write op";
                ...
                if (H5Dgather (chunk_info[i].mspace_id, buf[dsetnum],
                               mem_type_id[dsetnum],
                               (size_t)num_elem * mem_type_size, tconv_buf,
                               NULL, NULL)
                    < 0)
                  std::cerr << "can't gather data to conversion buffer";

                /* Perform type conversion */
                if (H5Tconvert (mem_type_id[dsetnum], dset->type_id,
                                (size_t)num_elem, tconv_buf, bkg_buf, dxpl_id)
                    < 0)
                  std::cerr << "can't perform type conversion";
              } /* end else */
...
/*---------------------------------------------------------------------------*/
herr_t
H5VL_dtio_dataset_get (void *dset, H5VL_dataset_get_args_t *args,
                       hid_t dxpl_id, void **req)
{
    ...
    case H5VL_DATASET_GET_SPACE_STATUS:
      {
        H5D_space_status_t *allocation
            = va_arg (arguments, H5D_space_status_t *);
                       ...
                       hid_t fapl_id, hid_t dxpl_id, void **req)
{
  ...
  file->flags = flags;
  file->max_oid = 0;
  if (H5VL_dtio_oid_create_string (file, file->max_oid, &file->glob_md_oid)
      < 0)
  ...
  if (NULL
      == (file->root_grp = (H5VL_dtio_group_t *)H5VL_dtio_group_create_helper (
              file, fcpl_id, H5P_GROUP_ACCESS_DEFAULT, dxpl_id, req, NULL,
              NULL, TRUE)))
  ...
  if (file->my_rank == 0)
    {
      ...
      if (NULL
          == (file->root_grp
              = (H5VL_dtio_group_t *)H5VL_dtio_group_open_helper (
      ...
      if (file->num_procs > 1)
        {
          ...
          if (gcpl_len + 2 * sizeof (uint64_t) > sizeof (foi_buf_static))
            {
              ...
              if (NULL
                  == (foi_buf_dyn
                      = (char *)malloc (gcpl_len + 2 * sizeof (uint64_t))))
                std::cerr
                    << "can't allocate space for global container handle";
          ...
          /* MPI_Bcast foi_buf */
          if (MPI_SUCCESS
              != MPI_Bcast (foi_buf, (int)sizeof (foi_buf_static), MPI_BYTE, 0,
                            file->comm))
          ...
          if (foi_buf == foi_buf_dyn)
            if (MPI_SUCCESS
                != MPI_Bcast ((char *)p, (int)(gcpl_len), MPI_BYTE, 0,
                              file->comm))
  ...
  else
    {
      ...
      if (gcpl_len + 2 * sizeof (uint64_t) > sizeof (foi_buf_static))
        {
          ...
          /* Receive info buffer */
          if (MPI_SUCCESS
              != MPI_Bcast (foi_buf_dyn, (int)(gcpl_len), MPI_BYTE, 0,
                            info->comm))
      ...
      if (NULL
          == (file->root_grp
              = (H5VL_dtio_group_t *)H5VL_dtio_group_reconstitute (
                  file, root_grp_oid, p, H5P_GROUP_ACCESS_DEFAULT, dxpl_id,
                  req)))
...
/*---------------------------------------------------------------------------*/
static herr_t
H5VL_dtio_file_specific (void *file, H5VL_file_specific_args_t *args,
                         hid_t dxpl_id, void H5VL_ATTR_UNUSED **req)
{
...
/*---------------------------------------------------------------------------*/
static void *
H5VL_dtio_group_create (void *_item, const H5VL_loc_params_t *loc_params,
                        const char *name, hid_t H5VL_ATTR_UNUSED lcpl_id,
                        ...
                        void **req)
{
  ...
  if (NULL
      == (grp = (H5VL_dtio_group_t *)H5VL_dtio_group_create_helper (
                      ...
                      void **req)
{
  ...
  if (!collective || (item->file->my_rank == 0))
      ...
      else
        {
          ...
          if (target_name[0] == '\0'
              || (target_name[0] == '.' && target_name[1] == '\0'))
            {
              ...
              if (H5Pencode2 (grp->gcpl_id, gcpl_buf, &gcpl_size, H5P_DEFAULT)
                  < 0)
      ...
      if (collective && (item->file->num_procs > 1))
        {
          ...
          /* MPI_Bcast ginfo_buf */
          if (MPI_SUCCESS
              != MPI_Bcast ((char *)ginfo_buf_static,
          ...
          if (gcpl_len + 2 * sizeof (uint64_t) > sizeof (ginfo_buf_static))
            if (MPI_SUCCESS
                != MPI_Bcast ((char *)gcpl_buf, (int)gcpl_len, MPI_BYTE, 0,
                              item->file->comm))
  ...
  else
    {
      /* Receive GCPL */
      if (MPI_SUCCESS
          != MPI_Bcast ((char *)ginfo_buf_static, sizeof (ginfo_buf_static),
                        MPI_BYTE, 0, item->file->comm))
        std::cerr << "can't bcast group info";
      ...
      if (NULL
          == (grp = (H5VL_dtio_group_t *)H5VL_dtio_group_reconstitute (
  ...
  for (i = 0; i < rank; i++)
    {
      if (snprintf (enc_buf + oid_off, oid_len - oid_off, "%016llX",
                    (long long unsigned)chunk_loc[i])
          != 16)
        HGOTO_ERROR (H5E_VOL, H5E_CANTINIT, FAIL,
                     "can't encode string object id");
      oid_off += 16;
    } /* end for */
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_write_max_oid
 *
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_file_flush
 *
 * Purpose:     Flushes a DTIO file.  Currently just writes the max oid.
 *
 * Return:      Success:        0
 *              Failure:        -1
 *
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_file_close_helper
 *
 * Purpose:     Closes a DTIO HDF5 file.
 *
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_read
 *
 * Purpose:     Recreates dtio_read(). We are trying to use all read_op
 *              calls and this lets us do that without duplicating all
 *              the DTIO boilerplate.
 *
 * Return:      Success:        0
 *              Failure:        -1
 *
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_write_full
 *
 * Purpose:     Recreates dtio_write_full(). We are trying to use all
 *              write_op calls and this lets us do that without duplicating
 *              all the DTIO boilerplate.
 *
 * Return:      Success:        0
 *              Failure:        -1
 *
 ...
 *
 * Purpose:     Recreates dtio_stat(). We are trying to use all read_op
 *              calls and this lets us do that without duplicating all
 *              the DTIO boilerplate.
 *
 * Return:      Success:        0
...
H5VL_dtio_stat (const char *oid, uint64_t *psize, time_t *pmtime)
{
...
#else
  {
    ...
    if ((ret
         = dtio_read_op_operate (read_op, io, oid, LIBDTIO_OPERATION_NOFLAG))
        < 0)
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_link_follow
 *
    ...
    case H5L_TYPE_SOFT:
      {
        ...
        if (NULL
            == (target_grp = H5VL_dtio_group_traverse (
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_link_follow_comp
 *
 * Purpose:     Follows the link in grp identified with name, and returns
...
H5VL_dtio_link_follow_comp (H5VL_dtio_group_t *grp, char *name,
                            size_t name_len, hid_t dxpl_id, void **req,
                            uint64_t *oid)
{
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_group_traverse
 *
 ...
 *              The group must be closed with H5VL_dtio_group_close().
 *
  ...
  while (next_obj)
    {
      ...
      if (H5VL_dtio_link_follow_comp (grp, *obj_name,
                                      (size_t)(next_obj - *obj_name), dxpl_id,
                                      req, &oid)
          < 0)
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_group_traverse_const
 *
 * Purpose:     Wrapper for H5VL_dtio_group_traverse for a const path.
 *
 * Return:      Success:        group object.
 *              Failure:        NULL
 *
...
H5VL_dtio_group_traverse_const (H5VL_dtio_item_t *item, const char *path,
                                hid_t dxpl_id, void **req,
                                const char **obj_name, void **gcpl_buf_out,
                                uint64_t *gcpl_len_out)
{
  H5VL_dtio_group_t *grp = NULL;
  char *tmp_path = NULL;
  ...
  if (NULL == (tmp_path = strdup (path)))
    HGOTO_ERROR (H5E_RESOURCE, H5E_CANTALLOC, NULL,
                 "can't duplicate path name");

  /* Forward the call to the non-const routine */
  if (NULL
      == (grp = H5VL_dtio_group_traverse (item, tmp_path, dxpl_id, req,
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_group_create_helper
 *
...
H5VL_dtio_group_create_helper (H5VL_dtio_file_t *file, hid_t gcpl_id,
                               ...
                               H5VL_dtio_group_t *parent_grp, const char *name,
                               hbool_t collective)
{
  H5VL_dtio_group_t *grp = NULL;
  void *gcpl_buf = NULL;
  ...
  if (!collective || (file->my_rank == 0))
    {
      ...
      if ((ret
           = H5VL_dtio_write_full (grp->obj.oid, (char *)gcpl_buf, gcpl_size))
          < 0)
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_group_open_helper
 *
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_group_reconstitute
 *
...
H5VL_dtio_group_reconstitute (H5VL_dtio_file_t *file, uint64_t oid,
                              uint8_t *gcpl_buf, hid_t gapl_id, hid_t dxpl_id,
                              void **req)
{
  H5VL_dtio_group_t *grp = NULL;
 ...
 */
static htri_t
H5VL_dtio_need_bkg (hid_t src_type_id, hid_t dst_type_id,
                    size_t *dst_type_size, hbool_t *fill_bkg)
{
    ...
    case H5T_COMPOUND:
      {
        ...
        for (i = 0; i < nmemb; i++)
          {
            /* Get member type */
            if ((memb_type_id = H5Tget_member_type (dst_type_id, (unsigned)i))
                < 0)
              HGOTO_ERROR (H5E_DATATYPE, H5E_CANTGET, FAIL,
            ...
            /* Get member name */
            if (NULL
                == (memb_name = H5Tget_member_name (dst_type_id, (unsigned)i)))
              HGOTO_ERROR (H5E_DATATYPE, H5E_CANTGET, FAIL,
            ...
            if ((src_memb_type_id
                 = H5Tget_member_type (src_type_id, (unsigned)src_i))
                < 0)
              HGOTO_ERROR (H5E_DATATYPE, H5E_CANTGET, FAIL,
                           "can't get compound member type");
...
H5VL_dtio_tconv_init (hid_t src_type_id, size_t *src_type_size,
                      hid_t dst_type_id, size_t *dst_type_size,
                      hbool_t *_types_equal, H5VL_dtio_tconv_reuse_t *reuse,
                      hbool_t *_need_bkg, hbool_t *fill_bkg)
{
  ...
  if (types_equal)
    ...
    *dst_type_size = *src_type_size;
  else
    {
      ...
      if ((need_bkg = H5VL_dtio_need_bkg (src_type_id, dst_type_id,
                                          dst_type_size, fill_bkg))
          < 0)
        HGOTO_ERROR (H5E_DATATYPE, H5E_CANTINIT, FAIL,
                     "can't check if background buffer needed");
 ...
 */
static herr_t
H5VL_dtio_get_selected_chunk_info (hid_t dcpl, hid_t file_space_id,
                                   hid_t mem_space_id,
                                   H5VL_dtio_select_chunk_info_t **chunk_info,
                                   size_t *chunk_info_len)
{
  ...
  if (H5Pget_chunk (dcpl, H5S_MAX_RANK, chunk_dims) < 0)
    HGOTO_ERROR (H5E_PLIST, H5E_CANTGET, FAIL,
                 "can't get chunking information");
  ...
  if (NULL
      == (_chunk_info = (H5VL_dtio_select_chunk_info_t *)malloc (
  ...
  if (FAIL
      == (space_same_shape
          = H5Sselect_shape_same (file_space_id, mem_space_id)))
      ...
      if (TRUE == intersect)
        {
          ...
          while (i > (info_buf_alloced / sizeof (*_chunk_info)) - 1)
            {
              if (NULL
                  == (_chunk_info = (H5VL_dtio_select_chunk_info_t *)realloc (
                          _chunk_info, 2 * info_buf_alloced)))
                HGOTO_ERROR (
                    H5E_DATASET, H5E_CANTALLOC, FAIL,
          ...
          if (H5Sselect_hyperslab (tmp_chunk_fspace_id, H5S_SELECT_AND,
                                   start_coords, NULL, chunk_dims, NULL)
              < 0)
            {
              H5Sclose (tmp_chunk_fspace_id);
          ...
          if (space_same_shape)
            {
               ...
               * selection */
              if (H5Sselect_copy (tmp_chunk_mspace_id, tmp_chunk_fspace_id)
                  < 0)
                {
                  H5Sclose (tmp_chunk_mspace_id);
                  HGOTO_ERROR (H5E_DATASPACE, H5E_CANTCOPY, FAIL,
              ...
              for (j = 0; j < (size_t)fspace_ndims; j++)
                {
                  ...
                  //                    hsize_t, hssize_t);
                  chunk_mem_space_adjust[j]
                      = chunk_file_space_adjust[j]
                        - (hssize_t)_chunk_info[i].chunk_coords[j];
                } /* end for */

              /* Adjust the selection */
              if (H5Shyper_adjust_s (tmp_chunk_mspace_id,
                                     chunk_mem_space_adjust)
                  < 0)
                {
                  H5Sclose (tmp_chunk_mspace_id);
                  HGOTO_ERROR (H5E_DATASPACE, H5E_CANTSELECT, FAIL,
          ...
          if ((chunk_sel_npoints = H5Sget_select_npoints (tmp_chunk_fspace_id))
              < 0)
            HGOTO_ERROR (
                H5E_DATASPACE, H5E_CANTGET, FAIL,
          ...
          do
            {
              ...
              start_coords[increment_dim]
                  = selection_start_coords[increment_dim];
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_build_io_op_merge
 *
 * Purpose:     DTIOINC
 *
 * Return:      Success:        0
...
H5VL_dtio_build_io_op_merge (hid_t mem_space_id, hid_t file_space_id,
                             size_t type_size, size_t tot_nelem, void *rbuf,
                             const void *wbuf, task read_op, task write_op)
{
  ...
  if ((file_sel_iter_id = H5Ssel_iter_create (file_space_id, type_size, 0))
      < 0)
    HGOTO_ERROR (H5E_DATASPACE, H5E_CANTINIT, FAIL,
                 "unable to initialize selection iterator");
  file_sel_iter_init
  ...
  do
    {
      ...
      if (mem_i == mem_nseq)
        {
          if (H5Ssel_iter_get_seq_list (
                  mem_sel_iter_id, (size_t)H5VL_DTIO_SEQ_LIST_LEN, (size_t)-1,
                  &mem_nseq, &nelem, mem_off, mem_len)
              < 0)
            HGOTO_ERROR (H5E_DATASPACE, H5E_CANTGET, FAIL,
      ...
      if (file_i == file_nseq)
        {
          if (H5Ssel_iter_get_seq_list (
                  file_sel_iter_id, (size_t)H5VL_DTIO_SEQ_LIST_LEN, (size_t)-1,
                  &file_nseq, &nelem, file_off, file_len)
              < 0)
            HGOTO_ERROR (H5E_DATASPACE, H5E_CANTGET, FAIL,
                         "sequence length generation failed");
          file_i = 0;
       ...
       * pair */
      io_len = mem_len[mem_i] <= file_len[file_i] ? mem_len[mem_i]
                                                  : file_len[file_i];

      /* Add to I/O op */
      if (rbuf)
        dtio_read_op_read (read_op, (uint64_t)(file_off[file_i] + file_ei),
                           io_len, (char *)rbuf + mem_off[mem_i] + mem_ei,
                           NULL, NULL);
      else
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_build_io_op_match
 *
 * Purpose:     DTIOINC
 *
 * Return:      Success:        0
  ...
  do
    {
      ...
      if (H5Ssel_iter_get_seq_list (sel_iter_id,
...
/*-------------------------------------------------------------------------
 * Function:    H5VL_dtio_build_io_op_contig
 *
 * Purpose:     DTIOINC
 *
 * Return:      Success:        0
