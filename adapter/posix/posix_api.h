...
extern "C"
{
  ...
  typedef ssize_t (*pwrite_t) (int fd, const void *buf, size_t count,
                               off_t offset);
  typedef ssize_t (*pread64_t) (int fd, void *buf, size_t count,
                                off64_t offset);
  ...
  // stat functions
  typedef int (*__fxstat_t) (int __ver, int __filedesc,
...
public:
  ...
  fd_whitelistedp (int fd)
  {
    if (interception_whitelist == nullptr)
      ...
      {
        return fd_interception_whitelist->find (fd)
               != fd_interception_whitelist->end ();
      }
  }
  ...
  getInstance ()
  {
    return instance == nullptr
               ? instance = std::make_shared<dtio::posix::PosixApi> ()
               : instance;
  }
};
...
/** Used for compatability with older kernel versions */
static int
fxstat_to_fstat (int fd, struct stat *stbuf)
{
#ifdef _STAT_VER
