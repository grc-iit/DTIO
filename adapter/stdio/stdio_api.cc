...
std::shared_ptr<dtio::stdio::StdioApi> dtio::stdio::StdioApi::instance = nullptr;
...
size_t HERMES_DECL(fwrite)(const void *ptr, size_t size, size_t count, FILE *stream) {
  DTIO_LOG_DEBUG_RANKLESS("Intercepted " << __func__);
  auto real_api = HERMES_STDIO_API;
  std::string caller_name = "";
  ...
  if (real_api->fp_whitelistedp(stream)) {
    if (ConfigManager::get_instance()->ASYNC) {
      return dtio::stdio::fwrite(ptr, size, count, stream); // TODO eventually, write_async
    }
    else {
