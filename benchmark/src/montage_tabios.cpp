...
int main(int argc, char **argv) {
        ...
        if (rank % 2 == 0 || comm_size == 1) {
          if (j % 2 == 0) {
            operations.emplace_back(std::make_pair(
                item[0],
                dtio::fwrite_async(write_buf[j], sizeof(char), item[0], fd1)));
          } else {
            operations.emplace_back(std::make_pair(
                item[0],
                dtio::fwrite_async(write_buf[j], sizeof(char), item[0], fd2)));
          }
        }
#ifdef TIMERBASE
        w.pauseTime();
  ...
  double w_time = 0.0;
  if (rank % 2 == 0) w_time = w.getElapsedTime();
  double w_sum;
...
#ifdef TIMERBASE
  double r_time = 0.0;
  if (rank % 2 == 1 || comm_size == 1) r_time = r.getElapsedTime();
  double r_sum;
