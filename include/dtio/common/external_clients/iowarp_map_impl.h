...
private:
  service map_service;
  // iowarp::unordered_map<struct IOWARPKeyType, std::string, std::hash<IOWARPKeyType>,
  //                    CharAllocator, MappedUnitString> *iowarp_client;
  std::string iowarpmapname;
  std::string get_server (std::string key, const table &tab);

  // iowarp::unordered_map<struct IOWARPKeyType, std::string, std::hash<IOWARPKeyType>, CharAllocator, MappedUnitString> *iowarp_string_client;
public:
  // iowarp::unordered_map<struct IOWARPKeyType, DTIOCharStruct> *iowarp_client;
  // iowarp::unordered_map<struct IOWARPKeyType, file_stat> *filestat_map; // I might need to move these to a separate process
  ...
  // Constructor
  IOWARPMapImpl (service service, std::string mapname) : distributed_hashmap (service) {
    iowarpmapname = mapname;

//     if (mapname == "dataspace") {
// #if(STACK_ALLOCATION)
//       iowarp_client = new iowarp::unordered_map<IOWARPKeyType, DTIOCharStruct> (mapname);
// #else
//       iowarp_string_client = new iowarp::unordered_map<IOWARPKeyType, std::string, std::hash<IOWARPKeyType>, CharAllocator, MappedUnitString>(mapname);
// #endif
//     }
...
//       filestat_map = new iowarp::unordered_map<IOWARPKeyType, file_stat> (mapname + "+fs");
//     }
//     else if (mapname == "metadata+chunkmeta") {
//       chunkmeta_map = new iowarp::unordered_map<IOWARPKeyType, chunk_meta> (mapname + "+chunkmeta");
//     }
//     else if (mapname == "metadata+filemeta") {
//       filemeta_map = new iowarp::unordered_map<IOWARPKeyType, file_meta> (mapname + "+filemeta");
//     }
