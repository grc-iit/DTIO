...
struct file
{
  ...
  //       // Parse location
  //       result.location =
  //       static_cast<location_type>(std::stoi(matches[1].str()));

  //       // Parse filename (with safety check for buffer size)
  //       std::strncpy(result.filename, matches[2].str().c_str(),
  //                   DTIO_FILENAME_MAX - 1);
  //       result.filename[DTIO_FILENAME_MAX - 1] = '\0';  // Ensure null
  //       termination
...
struct chunk_meta
{
  ...
  // {
  //   os << "actual_user_chunk = { " << arg.actual_user_chunk << " },
  //   destination = { " << arg.destination << " }"; return os;
  // }

  // chunk_meta from_string(const std::string& str) {
  //   chunk_meta result;

  //   // Regex pattern that matches the outer structure and captures the inner
  //   contents std::regex pattern(R"(actual_user_chunk = \{ (.*?) \},
  //   destination = \{ (.*?) \})");
  ...
  //       // Use the previously defined parse_file_regex function to parse
  //       each file struct result.actual_user_chunk =
  //       result.actual_user_chunk.from_string(actual_user_chunk_str);
  //       result.destination =
  //       result.destination.from_string(destination_str);
  //   } else {
...
struct file_meta
{
  ...
  //   std::cout << "Copying chunks in constructor from current index " <<
  //   current_chunk_index << std::endl; for (int i = 0; i < num_chunks_; i++)
  //   {
  ...
  //     os << "{ " << arg.chunks[i] << " }, ";
  //   }
  //   os << " ], current_chunk_index = " << arg.current_chunk_index << ",
  //   num_chunks = " << arg.num_chunks; return os;
  // }
  ...
  //     (.*?) \], )"
  // 			      R"(current_chunk_index = (-?\d+), num_chunks =
  // (-?\d+))");
  ...
  //       // Parse file_struct
  //       result.file_struct =
  //       result.file_struct.from_string(main_matches[1].str());
  ...
  //       int chunk_index = 0;
  //       while (std::regex_search(searchStart, searchEnd, chunk_matches,
  //       chunk_pattern)
  //              && chunk_index < CHUNK_LIMIT) {
  //           // Parse each chunk_meta
  //           result.chunks[chunk_index] =
  //           result.chunks[0].from_string(chunk_matches[1].str());
  //           chunk_index++;
  ...
  //       throw std::invalid_argument(std::string("Failed to parse file_meta:
  //       ") + e.what());
  //     }
...
struct file_stat
{
  ...
  // {
  //   os << "fh = " << arg.fh << ", fd = " << arg.fd << ", file_pointer = " <<
  //   arg.file_pointer <<
  //     ", file_size = " << arg.file_size << ", flags = " << arg.flags << ",
  //     arg.posix_mode = " << arg.posix_mode <<
  //     ", is_open = " << arg.is_open;
  //   return os;
  // }
  ...
  // MSGPACK_DEFINE(fd, file_pointer, file_size, flags, posix_mode, mode,
  // is_open);
