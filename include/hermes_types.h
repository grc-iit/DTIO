...
template <int TYPE> struct UniqueId
{
  ...
  bool
  operator== (const UniqueId &other) const
  {
...
template <int num>
std::ostream &
operator<< (std::ostream &os, UniqueId<num> const &obj)
{
...
union TargetId
{
  ...
  struct
  {
    ...
    i32 node_id_;
    /** The ID of the virtual device that backs this target. It is an index
  ...
  bool
  operator== (const TargetId &other) const
  {
...
struct PlacementSchema
{
  ...
  void
  AddSubPlacement (size_t size, TargetId tid)
  {
...
public:
  ...
  to_enum (const std::string &policy)
  {
...
namespace std
{
template <int TYPE> struct hash<hermes::UniqueId<TYPE> >
{
  std::size_t
  operator() (const hermes::UniqueId<TYPE> &key) const
  {
