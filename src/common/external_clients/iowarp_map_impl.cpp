...
{
  while (true)
    ; // Busy loop, the maps just support the data structures
}
                    ...
                    std::string group_key)
{
  if (name != table::FILE_CHUNK_DB)
    {
      DTIO_LOG_ERROR ("Attempt to put file stat into unrelated database "
                      << name);
    }
  std::string new_key = std::to_string (name) + KEY_SEPARATOR + key;
...
int
IOWARPMapImpl::put (const table &name, std::string key,
...
int
IOWARPMapImpl::put (const table &name, std::string key, const char *value,
                    ...
                    file_stat *result)
{
  ...
  if (name != table::FILE_DB)
    {
      DTIO_LOG_ERROR ("Attempt to get file stat from unrelated database "
                      << name);
    }
...
std::string
IOWARPMapImpl::remove (const table &name, std::string key,
                       std::string group_key)
{
  //   key = std::to_string (name) + KEY_SEPARATOR + key;
...
bool
IOWARPMapImpl::exists (const table &name, std::string key,
                       std::string group_key)
{
  key = std::to_string (name) + KEY_SEPARATOR + key;
  CHIMAERA_CLIENT_INIT ();
