...
public:
  ...
  std::shared_ptr<io_client> clients[io_client_type::MULTI];

  multi_client(int worker_index) : io_client(worker_index) {
    dir = ConfigManager::get_instance()->WORKER_PATH + "/" +
          std::to_string(worker_index) + "/";
    clients[io_client_type::STDIO] = std::make_shared<stdio_client>(stdio_client(worker_index));
    clients[io_client_type::POSIX] = std::make_shared<posix_client>(posix_client(worker_index));
    clients[io_client_type::URING] = std::make_shared<uring_client>(uring_client(worker_index));
    clients[io_client_type::HDF5] = std::make_shared<hdf5_client>(hdf5_client(worker_index));
  }
