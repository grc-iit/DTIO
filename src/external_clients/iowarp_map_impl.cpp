...
int IOWARPMapImpl::run() {
  while (true);  // Busy loop, the maps just support the data structures
}

int IOWARPMapImpl::put(const table &name, std::string key, chunk_meta *data,
                       std::string group_key) {
  if (name != table::CHUNK_DB) {
    DTIO_LOG_ERROR("Attempt to put chunk meta into unrelated database "
                   << name);
  }
  ...
  hipc::FullPtr<char> mykey = CHI_CLIENT->AllocateBuffer(HSHM_MCTX, keysize);
  hipc::FullPtr<char> myval = CHI_CLIENT->AllocateBuffer(HSHM_MCTX, valsize);
  ...
  client.MetaPut(
      ...
      mykey.shm_, keysize, myval.shm_, valsize);

  return 0;
}

int IOWARPMapImpl::put(const table &name, std::string key, file_meta *fm,
                       std::string group_key) {
...
int IOWARPMapImpl::put(const table &name, std::string key, file_stat *fs,
                       std::string group_key) {
...
int IOWARPMapImpl::put(const table &name, std::string key,
...
int IOWARPMapImpl::put(const table &name, std::string key, const char *value,
...
std::string IOWARPMapImpl::get(const table &name, std::string key,
                               std::string group_key) {
...
void IOWARPMapImpl::get(const table &name, std::string key,
...
bool IOWARPMapImpl::get(const table &name, std::string key,
                        std::string group_key, chunk_meta *result) {
  if (name != table::CHUNK_DB) {
    DTIO_LOG_ERROR("Attempt to get chunk meta from unrelated database "
                   << name);
  }
...
bool IOWARPMapImpl::get(const table &name, std::string key,
                        std::string group_key, file_meta *result) {
  if (name != table::FILE_CHUNK_DB) {
    DTIO_LOG_ERROR("Attempt to get file chunk meta from unrelated database "
                   << name);
  }
...
std::string IOWARPMapImpl::remove(const table &name, std::string key,
                                  std::string group_key) {
  //   key = std::to_string (name) + KEY_SEPARATOR + key;
...
bool IOWARPMapImpl::exists(const table &name, std::string key,
                           std::string group_key) {
  key = std::to_string(name) + KEY_SEPARATOR + key;
  CHIMAERA_CLIENT_INIT();
  chi::dtiomod::Client client;
...
bool IOWARPMapImpl::purge() {
  // Not necessary, nothing calls it at the moment and memcached just does a
  // flush
  return true;
}

size_t IOWARPMapImpl::counter_init(const table &name, std::string key,
                                   std::string group_key) {
  key = std::to_string(name) + KEY_SEPARATOR + key;
  if (group_key == "-1") {
...
size_t IOWARPMapImpl::counter_inc(const table &name, std::string key,
                                  std::string group_key) {
...
std::string IOWARPMapImpl::get_server(std::string key, const table &tab) {
  // auto true_key = IOWARPKeyType (key);
