...
int dtio::hdf5::DTIO_open(const char *filename, const char *dsetname,
  ...
  if (dsetname == NULL) {
    ...
    path =
        (char *)calloc(strlen(filename) + strlen(dsetname) + 2, sizeof(char));
...
int dtio::hdf5::DTIO_write(const char *filename, const char *dsetname,
...
int dtio::hdf5::DTIO_read(const char *filename, const char *dsetname, char *buf,
...
#if (STACK_ALLOCATION)
    {
      ...
      switch (t.source.location) {
        ...
        case CACHE: {
          ...
          data_m->get(DATASPACE_DB, t.source.filename,
                      std::to_string(t.destination.server), char_data);
          DTIO_LOG_TRACE("Doing memcpy " << ptr_pos << " " << t.source.offset
                                         << " " << t.destination.size << " "
                                         << t.source.size << std::endl);
...
#else
    {
      ...
      switch (t.source.location) {
        ...
        case BUFFERS: {
          ...
          auto map_fm_client =
              dtio_system::getInstance(WORKER)->map_client("metadata+filemeta");
          ...
          for (int i = 0; i < fm.num_chunks; ++i) {
            ...
            if (fm.current_chunk_index - i - 1 >= 0) {
              ...
              // << std::endl;
              curr_chunk = &(
                  fm.chunks[fm.current_chunk_index - i - 1].actual_user_chunk);

              // std::cout << "Check 2" << std::endl;
              if (dtio_system::getInstance(WORKER)->range_resolve(
                      &range_bound, t.source.size, range_lower, range_upper,
                      curr_chunk->offset, curr_chunk->offset + curr_chunk->size,
                      &range_resolved)) {
              ...
              curr_chunk =
                  &(fm.chunks[CHUNK_LIMIT + fm.current_chunk_index - i - 1]
                        .actual_user_chunk);
              // std::cout << "Check 2" << std::endl;
              if (dtio_system::getInstance(WORKER)->range_resolve(
                      &range_bound, t.source.size, range_lower, range_upper,
          ...
          if (range_resolved) {
            ...
            map_client->put(DATASPACE_DB, t.source.filename, char_data,
                            MAX_IO_UNIT, std::to_string(t.destination.server));
            free(char_data);
            continue;
          }
