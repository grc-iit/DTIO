 ...
 ******************************************************************************/
FILE *
dtio::stdio::fopen (const char *filename, const char *mode)
{
      ...
      if (strcmp (mode, "a") == 0 && strcmp (mode, "w") == 0)
        {
          ...
          if (!file_exists_in_fs)
            ...
            {
              if (mdm->update_on_open (filename, mode, fh, existing_size)
                  != SUCCESS)
                {
...
int
dtio::stdio::fseek (FILE *stream, long int offset, int origin)
{
  auto mdm = metadata_manager::getInstance (LIB);
  if (!mdm->is_opened (stream))
    {
  ...
  // FIXME may require bigger metadata adjustments, we don't seem to track
  // STDIO mode yet
...
size_t
dtio::stdio::fread (void *ptr, size_t size, size_t count, FILE *stream,
                    bool special_type)
{
  auto mdm = metadata_manager::getInstance (LIB);
  ...
  size_t data_size = hshm::Unit<size_t>::Megabytes (
      (size_t)ceil ((double)(size * count) / 1024.0));
  ...
  //     }
  // }
  // auto r_task = task(task_type::READ_TASK, file(filename, offset, size *
  // count), file()); r_task.check_fs = check_fs; r_task.special_type =
  // special_type; if (ConfigManager::get_instance()->USE_CACHE) {
...
size_t
dtio::stdio::fwrite (const void *ptr, size_t size, size_t count, FILE *stream)
{
  ...
  // auto write_tasks = task_m->build_write_task(w_task, static_cast<const char
  // *>(ptr));
