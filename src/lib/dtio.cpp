...
int
dtio::DTIO_write (std::string filename, std::string buf, int64_t offset,
                  size_t count)
{
  ...
  auto write_tasks = task_m->build_write_task (w_task, buf.data ());
#ifdef TIMERTB
  std::stringstream stream1;
  stream1 << "build_write_task()," << t.pauseTime () << "\n";
          ...
          if (write_data.length () >= tsk->source.offset + tsk->source.size)
            {
              ...
              data_m->put (DATASPACE_DB, tsk->source.filename, write_data_char,
                           count, std::to_string (tsk->destination.server));
...
std::string
dtio::DTIO_read (std::string filename, int64_t offset, size_t count)
{
  auto mdm = metadata_manager::getInstance (LIB);
  //   auto client_queue
...
#ifdef TIMERTB
  std::stringstream stream1;
  stream1 << "build_read_task()," << t.pauseTime () << "\n";
  DTIO_LOG_TRACE (stream1.str ());
#endif
  int ptr_pos = 0;
  size_t size_read = 0;
              ...
              if (ConfigManager::get_instance ()->CHECKFS)
                {
                  ...
                  DTIO_LOG_INFO ("Get data into "
                                 << ptr_pos << " from " << t.source.offset
                                 << " size " << t.destination.size);
          ...
          case BUFFERS:
            {
              ...
              DTIO_LOG_INFO ("Get data into " << ptr_pos << " from "
                                              << t.source.offset << " size "
                                              << t.destination.size);
