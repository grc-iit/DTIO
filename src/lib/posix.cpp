...
int dtio::posix::open(const char *filename, int flags) {
  ...
  if (!mdm->is_created(filename)) {
    if (!(flags & O_CREAT)) {
      ...
      if (ConfigManager::get_instance()->CHECKFS) {
        struct stat st;
        if (stat((strncmp(filename, "dtio://", 7) == 0) ? (filename + 7)
                                                        : filename,
                 &st) == 0) {
...
int dtio::posix::open(const char *filename, int flags, mode_t mode) {
  ...
  if (!mdm->is_created(filename)) {
    if (!(flags & O_CREAT)) {
      ...
      if (!file_exists_in_fs) {
        ...
        if (mdm->update_on_open(filename, flags, mode, &fd, existing_size) !=
            SUCCESS) {
          throw std::runtime_error("dtio::posix::open() update failed!");
        }
        publish_task = true;
...
int dtio::posix::open64(const char *filename, int flags) {
...
int dtio::posix::open64(const char *filename, int flags, mode_t mode) {
...
int dtio::posix::unlink(const char *pathname) {
  ...
  for (auto task : delete_tasks) {
    if (task.publish) {
      ...
      task_ids.emplace_back(std::make_pair(task.source.filename,
...
int dtio::posix::fsync(int fd) {
  // We don't want to do anything for flush
  return 0;
}

int dtio::posix::rename(const char *oldpath, const char *newpath) {
  // FIXME not implemented
  return 0;
}

int dtio::posix::mystat(const char *pathname, struct stat *statbuf) {
  ...
  DTIO_LOG_TRACE("[DTIO][POSIX][STAT] " << pathname << " " << statbuf->st_size);
  return 0;
}

int dtio::posix::myfstat(int fd, struct stat *statbuf) {
  // Note: I actually had to rename stat in order to use it for file existence
  // checks
  // FIXME right now we just grab file size, which is ok for IOR but also bad
  auto mdm = metadata_manager::getInstance(LIB);
...
int dtio::posix::myfstat64(int fd, struct stat64 *statbuf) {
  // Note: I actually had to rename stat in order to use it for file existence
  // checks
  // FIXME right now we just grab file size, which is ok for IOR but also bad
  auto mdm = metadata_manager::getInstance(LIB);
...
int dtio::posix::mystat64(const char *pathname, struct stat64 *statbuf) {
  // FIXME right now we just grab file size, which is ok for IOR but also bad
  auto mdm = metadata_manager::getInstance(LIB);
  if (!mdm->is_created(pathname)) {
    return -1;
...
int dtio::posix::mknod(const char *pathname, mode_t mode, dev_t dev) {
  // FIXME not implemented
  return 0;
}

int dtio::posix::close(int fd) {
  auto mdm = metadata_manager::getInstance(LIB);
  if (!mdm->is_opened(fd)) {
...
off_t dtio::posix::lseek(int fd, off_t offset, int whence) {
  auto mdm = metadata_manager::getInstance(LIB);
  if (!mdm->is_opened(fd)) {
    return EBADF;
  }
...
ssize_t dtio::posix::read(int fd, void *buf, size_t count) {
  auto mdm = metadata_manager::getInstance(LIB);
  auto task_m = dtio_system::getInstance(LIB)->task_composer();
  auto data_m = data_manager::getInstance(LIB);
...
ssize_t dtio::posix::write(int fd, const void *buf, size_t count) {
...
int dtio::posix::__fxstat64(int __ver, int fd,
                            struct stat64 *buf) {  // TODO: implement
  myfstat64(fd, buf);
  return 0;
}

int dtio::posix::__open_2(const char *path, int oflag) {
  return open(path, oflag);
}

int dtio::posix::__fxstat(int __ver, int fd,
                          struct stat *buf) {  // TODO: implement
  myfstat(fd, buf);
  return 0;
}

int dtio::posix::__fxstatat(int __ver, int __fildes, const char *__filename,
                            struct stat *__stat_buf,
                            int __flag) {  // TODO: implement
  return 0;
}

int dtio::posix::__xstat(int __ver, const char *__filename,
                         struct stat *__stat_buf) {  // TODO: implement
  return 0;
}

int dtio::posix::__xstat64(int __ver, const char *__filename,
                           struct stat64 *__stat_buf) {  // TODO: implement
  return 0;
}

int dtio::posix::__lxstat(int __ver, const char *__filename,
                          struct stat *__stat_buf) {  // TODO: implement
  return 0;
}

int dtio::posix::__fxstatat64(int __ver, int __fildes, const char *__filename,
                              struct stat64 *__stat_buf,
                              int __flag) {  // TODO: implement
  return 0;
}

int dtio::posix::__lxstat64(int __ver, const char *__filename,
                            struct stat64 *__stat_buf) {  // TODO: implement
  return 0;
}
