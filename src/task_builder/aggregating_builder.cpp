...
void
aggregating_builder::close_aggregation ()
{
  ...
  */
  ...
  while (aggregation_tasks.size () > 0)
    {
      ...
      sub_task->source.offset
          = std::min (sub_task->source.offset, agg_tsk->source.offset);
      sub_task->destination.offset = std::min (sub_task->destination.offset,
                                               agg_tsk->destination.offset);
  ...
  map_client->put (table::DATASPACE_DB, sub_task->source.filename,
                   aggregate_buffer + sub_task->source.offset,
                   sub_task->destination.size, std::to_string (-1));

  mdm->update_write_task_info (*sub_task, sub_task->destination.filename,
                               sub_task->destination.size);
  ...
  while (!map_server->exists (table::WRITE_FINISHED_DB,
                              std::to_string (sub_task->task_id),
                              std::to_string (-1)))
    {
    }
  map_server->remove (table::WRITE_FINISHED_DB,
                      std::to_string (sub_task->task_id), std::to_string (-1));
 ...
 ******************************************************************************/
std::vector<task *>
aggregating_builder::build_write_task (task tsk, const char *data)
{
  auto map_client = dtio_system::getInstance (service_i)->map_client ();
  ...
  if (aggregation_offset != tsk.destination.offset)
    {
      DTIO_LOG_DEBUG ("Closing aggregate due to seek "
                      << aggregation_offset
                      << " != " << tsk.destination.offset);
      close_aggregation ();
    }
  if (aggregation_tasks.size () != 0
      && strcmp (tsk.destination.filename,
                 aggregation_tasks.back ()->destination.filename)
             != 0)
    {
      DTIO_LOG_DEBUG ("Closing aggregate due to filename change "
                      << tsk.destination.filename << " != "
                      << aggregation_tasks.back ()->destination.filename);
      close_aggregation ();
    }
  ...
  if (remaining_data < MIN_IO_UNIT)
    {
      ...
      // source filename is used to indicate chunk, because buffer needs to be
      // MAX_IO_UNIT
      sprintf (sub_task->source.filename, "%s%i", tsk.destination.filename,
               chunk_index);
      ...
      memcpy (aggregate_buffer + current_aggregate_size,
              data + sub_task->source.offset, remaining_data);
      if (sub_task->destination.size + current_aggregate_size >= MIN_IO_UNIT)
        {
          // Aggregation trigger
          sub_task->destination.size
              = sub_task->destination.size
                + current_aggregate_size; // Aggregate size
          sub_task->source.size = sub_task->destination.size;
          while (aggregation_tasks.size () > 0)
            {
              // Smallest offset
              ...
              sub_task->source.offset
                  = std::min (sub_task->source.offset, agg_tsk->source.offset);
              sub_task->destination.offset = std::min (
                  sub_task->destination.offset, agg_tsk->destination.offset);
              aggregation_tasks.pop_back ();
            }
          map_client->put (table::DATASPACE_DB, sub_task->source.filename,
                           aggregate_buffer + data_offset
                               + sub_task->source.offset,
                           sub_task->destination.size, std::to_string (-1));
          sub_task->publish = true;
      ...
      else
        {
          ...
          aggregation_offset
              = sub_task->destination.offset + sub_task->destination.size;
        }
    }
  else
    {
      while (remaining_data > 0)
        {
          ...
          if (remaining_data >= MAX_IO_UNIT)
            {
              sub_task->source = file (tsk.source);
              // source filename is used to indicate chunk, because buffer
              // needs to be MAX_IO_UNIT
              sprintf (sub_task->source.filename, "%s%i",
                       tsk.destination.filename, chunk_index);
              chunk_index++;
              sub_task->source.offset = data_offset + tsk.source.offset;
              sub_task->source.size = MAX_IO_UNIT;
              sub_task->destination = file (tsk.destination);
              sub_task->destination.offset
                  = data_offset + tsk.destination.offset;
              sub_task->destination.size = MAX_IO_UNIT;

              map_client->put (table::DATASPACE_DB, sub_task->source.filename,
                               data + data_offset + tsk.source.offset,
                               sub_task->destination.size,
                               std::to_string (-1));
              sub_task->addDataspace = false;
...
std::vector<task>
aggregating_builder::build_read_task (task t)
{
  auto tasks = std::vector<task> ();
  auto mdm = metadata_manager::getInstance (LIB);
  auto map_server = dtio_system::getInstance (service_i)->map_server ();
  ...
  //   snprintf(rt->destination.filename, DTIO_FILENAME_MAX, "%d",
...
std::vector<task>
aggregating_builder::build_delete_task (task tsk)
{
  auto tasks = std::vector<task> ();
  auto mdm = metadata_manager::getInstance (LIB);
  auto map_server = dtio_system::getInstance (service_i)->map_server ();
