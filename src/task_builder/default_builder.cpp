 ...
 ******************************************************************************/
std::vector<task *>
default_builder::build_write_task (task tsk, const char *data)
{
      ...
      if (remaining_data >= MAX_IO_UNIT)
        {
          ...
          // source filename is used to indicate chunk, because buffer needs to
          // be MAX_IO_UNIT
          sprintf (sub_task->source.filename, "%s%i", tsk.destination.filename,
                   dataspace_id % CHUNK_LIMIT);
          dataspace_id = map_server->counter_inc (COUNTER_DB, DATASPACE_ID,
                                                  std::to_string (-1));
...
std::vector<task>
default_builder::build_read_task (task t)
{
  auto tasks = std::vector<task> ();
  ...
  // FIXME DataTasks are supposed to get split up into smaller DataTasks for
  // parsing. This logic was removed when chunking was reorganized, but it
  // needs to be reintroduced eventually. For now, let's simply split reads
  ...
  if (!t.source.location == BUFFERS && t.source.size > MAX_IO_UNIT)
    {
      // Sometimes we have to split read tasks, but try to avoid it when we do
      // caching
      while (remaining_data > 0)
        {
          ...
          if (remaining_data >= MAX_IO_UNIT)
            {
              ...
              // dest filename is used to indicate chunk, because buffer needs
              // to be MAX_IO_UNIT
              sprintf (sub_task->destination.filename, "%s%i",
                       t.source.filename, dataspace_id % CHUNK_LIMIT);
              dataspace_id = map_server->counter_inc (COUNTER_DB, DATASPACE_ID,
                                                      std::to_string (-1));
              sub_task->destination.offset
                  = data_offset + t.destination.offset;
              sub_task->destination.size = MAX_IO_UNIT;
          ...
          else
            {
              ...
              // to be MAX_IO_UNIT
              sprintf (sub_task->destination.filename, "%s%i",
                       t.destination.filename, dataspace_id % CHUNK_LIMIT);
              dataspace_id = map_server->counter_inc (COUNTER_DB, DATASPACE_ID,
                                                      std::to_string (-1));
              sub_task->destination.offset
                  = data_offset + t.destination.offset;
              ...
              // map_client->put(table::DATASPACE_DB,
              // sub_task->source.filename, data + data_offset +
              // tsk.source.offset, sub_task->destination.size,
              // std::to_string(-1));
              sub_task->addDataspace = false;
...
std::vector<task>
default_builder::build_delete_task (task tsk)
{
  auto tasks = std::vector<task> ();
  auto mdm = metadata_manager::getInstance (LIB);
  auto map_server = dtio_system::getInstance (service_i)->map_server ();
